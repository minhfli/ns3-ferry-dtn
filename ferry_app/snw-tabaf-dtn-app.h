...
class SNW_TABAF_DtnApp : public EpidemicBaseDtnApp {
    public:
    SNW_TABAF_DtnApp() : EpidemicBaseDtnApp() {};
    virtual ~SNW_TABAF_DtnApp() {};

    static TypeId GetTypeId(void) {
        static TypeId tid = TypeId("ns3::SNW_TABAF_DtnApp")
            .SetParent<EpidemicBaseDtnApp>()
            .AddConstructor<SNW_TABAF_DtnApp>();
        return tid;
    }
...
NS_OBJECT_ENSURE_REGISTERED(SNW_TABAF_DtnApp);

void SNW_TABAF_DtnApp::InitializeMobility(const std::vector<uint32_t>& servingNodesIndex) {
    m_nodeScore.resize(config.nGrounds, 0);
    Simulator::Schedule(Seconds(0), &SNW_TABAF_DtnApp::ScheduleNextWaypoint, this);
}

void SNW_TABAF_DtnApp::ScheduleNextWaypoint() {
    ...
    m_mobilityScheduleEvent = Simulator::Schedule(Seconds(timeToReach), &SNW_TABAF_DtnApp::HoverAndScheduleNextWaypoint, this);

}

void SNW_TABAF_DtnApp::CalculateNodeScore() {
...
void SNW_TABAF_DtnApp::ChooseNextServingNode() {
...
void SNW_TABAF_DtnApp::BundleAckMobilityCallBack(Bundle b) {
...
std::vector<uint32_t> SNW_TABAF_DtnApp::GetServingNodeRoute() {
    return std::vector<uint32_t>{ m_nextServingNode };
}

std::vector<point2D> SNW_TABAF_DtnApp::GetServingWaypointRoute() {
    return std::vector<point2D>{ nodePos(m_nextServingNode)};
}

// Bundle* SNW_TABAF_DtnApp::FerrySelectBundleToFerry(Ipv4Address neighborIp) {
